...
export async function uploadReferenceImage(file: File, themeCode?: string): Promise<ReferenceImage> {
...
export async function generateDesign(request: DesignRequest, styleOverride?: File): Promise<DesignGenerationResponse> {
  ...
  const stringFields: (keyof DesignRequest)[] = [
    'productSegment', 'priceBand', 'polkiSize', 'motifCategory',
    'enamel', 'finish', 'designShape', 'materialRatio',
  ...
  if (request.stoneColour && request.stoneColour.length > 0) {
    formData.append('stoneColour', JSON.stringify(request.stoneColour));
  }
...
export interface CADComparisonResult {
  gemini: { imageUrl: string; model: string };
  openai: { imageUrl: string; model: string };
  prompt: string;
}
...
export async function modifyDesign(
  ...
  if (params.stoneColour && params.stoneColour.length > 0) {
    formData.append('stoneColour', JSON.stringify(params.stoneColour));
  }
